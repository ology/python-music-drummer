...
class TestDrummer(unittest.TestCase):
    ...
    def test_patterns(self):
        d = Drummer()
        d.count_in()
        self.assertEqual(len(d.cymbals.getElementsByClass('Note')), d.beats)
        d.pattern(patterns={'kick': '1000000010100000', 'snare': '0000100000001000', 'cymbals': '1010101010101010'})
        self.assertEqual(len(d.kick.getElementsByClass('Note')), 3)
        self.assertEqual(len(d.snare.getElementsByClass('Note')), 2)
        self.assertEqual(len(d.cymbals.getElementsByClass('Note')), 4 + 8)
        d.sync_parts()
